...
def plot_host_quasar(data,Lquasar,axes,err_axes,f,dust=False,colorbar=False):
  ...
  img2 = err_axes.imshow(1/((bkg_sigma)**2+(data_sigma)**2),cmap='magma', norm=LogNorm(vmin = mx_var/1e5, vmax=mx_var/10))#, vmin = mx -3, vmax=mx)
 
  hdu=fits.PrimaryHDU(img_data+img_bkg_data)
  hdu.writeto('data/sci_mock_F200W.fits',overwrite=True)
  
  hdu_ivm=fits.PrimaryHDU(1/((bkg_sigma)**2+(data_sigma)**2))
  hdu_ivm.writeto('data/ivm_mock_F200W.fits',overwrite=True)
