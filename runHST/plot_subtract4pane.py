...
def plot_models(quasar, save_name=None):
    qdir = quasar.split('_', 1)[0]
    stamp = fits.getdata(_stamp_pat.format(qdir, quasar))
    #stamp = fits.getdata(_stamp_pat)
    wcs = WCS(fits.getheader(_stamp_pat.format(qdir, quasar)))
    model = fits.getdata(_model_pat.format(qdir, quasar))
    #psfresid = fits.getdata(_psfresid_pat.format(qdir, quasar))
    rawmodel = fits.getdata(_rawmodel_pat.format(qdir, quasar))
    filt = 'F160W'
    residual = fits.getdata(_resid_pat.format(qdir, quasar))
