...
def plot_PSF(LPSF,axes,ivm_axes,f,err):
  ...
  #ivm=1/(bkg_sigma**2+data_sigma**2)
  ...
  img2 = err_axes.imshow(mask*ivm,cmap='magma', norm=LogNorm(vmin = mx_var/1e2, vmax=mx_var))#, vmin = mx -3, vmax=mx)
 
  
  hdu=fits.PrimaryHDU(mask*(img_data))#+img_bkg_data))
  
  if err>0.:
    hdu.writeto('data/sci_PSF_HST_{}'.format(err).replace('.','p')+'.fits',overwrite=True)
  else:
    hdu.writeto('data/sci_PSF_HST.fits',overwrite=True)
  ...
  if err>0.:
    hdu_ivm.writeto('data/ivm_PSF_HST_{}'.format(err).replace('.','p')+'.fits',overwrite=True)
  else:
    hdu_ivm.writeto('data/ivm_PSF_HST.fits',overwrite=True)
