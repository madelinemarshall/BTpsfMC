...
_psfresid_pat = 'runJWST/{}/mcmc_out_mock_{}_point_source_subtracted.fits'
...
_pnorm = ImageNormalize(vmin=-0.00001, vmax=0.03, stretch=_stretch, clip=True)
...
def plot_models(quasar, folder, save_name=None):
    qdir = 'JWST_F200W_'+quasar.split('_',1)[1]#+'_host'
    psfresid = fits.getdata(_psfresid_pat.format(folder,quasar))
...
if __name__ == '__main__':
    ...
    for quasar in to_plot:
        ...
        plot_models(quasar, folder, save_name=save_name)
        ii+=1
    ...
    #pp.savefig('SDSS_z7_residual_star.pdf')
    pp.show() 
    pp.close(fig)
