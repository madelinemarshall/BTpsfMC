...
def hist_x(data,mask,ax,**kwargs):
    ...
    bins_width=(bins_max-bins_min)/6
    for bb in range(0,6):
        ...
        if len(data[(data<=b_max) & (data>b_min)])>0:
          ...
          frac[bb]=np.nan
    ax.bar(bins_min+bins_width*(0.5+np.arange(0,6)),height=frac,width=bins_width,edgecolor='k',facecolor='#d6f1ff')#[0.8,0.8,0.8])
...
def hist_y(data,mask,ax,**kwargs):
    ...
    for bb in range(0,6):
        ...
        if len(data[(data<=b_max) & (data>b_min)])>0:
          ...
          frac[bb]=np.nan
    ax.barh(bins_min+bins_width*(0.5+np.arange(0,6)),width=frac,height=bins_width,edgecolor='k',facecolor='#d6f1ff')#[0.8,0.8,0.8])
...
def two_pane_plot(xdata,ydata,mask,ax,hist_ax,bottom=False):
    ax.plot(xdata[mask],ydata[mask],'ko',label='Detectable')
    ax.plot(xdata[~mask],ydata[~mask],'ko',markerfacecolor='w',label='Undetectable')
    if bottom:
      hist_x(xdata,mask,bottom,color='k')
    hist_y(ydata,mask,hist_ax,color='k')
...
def three_pane_plot(xdata,ydata,mask,ax,color='k'):
    ax[0,1].plot(xdata[mask],ydata[mask],'o',label='Detectable',color=color)#,color='#984ea3')         #['#e41a1c','#377eb8','#4daf4a','#984ea3')
    ax[0,1].plot(xdata[~mask],ydata[~mask],'o',markerfacecolor='w',label='Undetectable',color=color)#,color='#984ea3')
    hist_x(xdata,mask,ax[1,1])
    hist_y(ydata,mask,ax[0,0])
...
if __name__=='__main__':
    ...
    print(df2.columns)
    #detectable_CO=np.array(df2['F200W'],dtype=int)
    ...
    for ii,prop in enumerate(['BHMass','SFR']): 
      ...
      xdata=np.log10(df1['StellarMass'])
      ydata=np.log10(df1[prop])
      two_pane_plot(xdata,ydata,detectable_SDSS,ax[ii,1],ax[ii,0])
      ax[ii,0].set_ylabel(labs[prop])
      #ax[ii,1].set_xlabel(r'$\log(M_\ast/M_\odot)$')
    
    xdata=np.log10(df1['StellarMass'])
    ydata=df1['Radius']
    two_pane_plot(xdata,ydata,detectable_SDSS,ax[2,1],ax[2,0],bottom=ax[3,1])
    ax[3,0].axis('off')
    ...
    xdata=df1['MUV_AGN']+df1['tau_UV_AGN']
    ydata=df1['MUV_gal_dust']
    three_pane_plot(xdata,ydata,detectable_SDSS,ax)

    xdata=df2['MUV_AGN']+df2['tau_UV_AGN']
    ydata=df2['MUV_gal_dust']
    three_pane_plot(xdata,ydata,detectable_CO,ax,'r')
