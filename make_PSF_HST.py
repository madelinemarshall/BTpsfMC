...
def plot_PSF(LPSF,axes,ivm_axes,f):
  ...
  mx=np.amax(img_data)
  mx_var=np.amax(1/(bkg_sigma)**2)
 
  img1 = axes.imshow(mask*(img_data+img_bkg_data),cmap='magma', norm=LogNorm(vmin = mx/10000, vmax=mx))
  img2 = err_axes.imshow(mask*1/((bkg_sigma)**2+(data_sigma)**2),cmap='magma', norm=LogNorm(vmin = mx_var/1e5, vmax=mx_var/10))#, vmin = mx -3, vmax=mx)
 
  hdu=fits.PrimaryHDU(mask*(img_data))#+img_bkg_data))
  hdu.writeto('data/sci_PSF_HST.fits',overwrite=True)
  
  #hdu_ivm=fits.PrimaryHDU(mask*1/((bkg_sigma)**2+(data_sigma)**2))
...
if __name__=='__main__':
    ...
    width=8.33 #size of cutout in ''  #MUST RESULT IN EVEN NUMBER OF PIXELS
